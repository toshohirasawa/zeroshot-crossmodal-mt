...
for L3 in de fr cs tr ja; do
    ...
    for IM_LIST in $(ls ./data/en-${L2}_${L3}/image_splits/*); do
        ...
        for FEAT_NAME in $(ls ${FEAT_ROOT}); do
            [ "${FEAT_NAME}" != "resnet50" ] && continue
            echo "--> ${L2}_${L3} $(basename ${IM_LIST}) ${FEAT_NAME}"

            FEAT_FILE=./data/en-${L2}_${L3}/feats/${FEAT_NAME}-global/$(basename ${IM_LIST} .txt).npy
            FEAT_TARGET=${FEAT_ROOT}/${FEAT_NAME}/stack/${FEAT_ID}-global.npy
            ...
            ln -fs $(realpath "${FEAT_TARGET}") "${FEAT_FILE}"
        done
    done
done
done
